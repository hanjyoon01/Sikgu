...
const MODEL_MAP = {
  cactus_in_pot: {
    label: 'Cactus in Pot',
    url: '/models/Cactus_in_Pot_0924021319_texture.glb',
    targetHeight: 2.5,
  },
  cactus_serenity: {
    label: 'Cactus Serenity',
    url: '/models/Cactus_Serenity_0911025405_texture.glb',
    targetHeight: 1.5,
  },
  monstera_serenity: {
    label: 'Monstera Serenity',
    url: '/models/Monstera_Serenity_0924014751_texture.glb',
    targetHeight: 2.5,
  },
  peaceful_bloom: {
    label: 'Peaceful Bloom',
    url: '/models/Peaceful_Bloom_0911044801_texture.glb',
    targetHeight: 2.5,
  },
  philodendron_majesty: {
    label: 'Philodendron Majesty',
    url: '/models/Philodendron_Majesty_0909081725_texture.glb',
    targetHeight: 2.5,
  },
  potted_green_serenity: {
    label: 'Potted Green Serenity',
    url: '/models/Potted_Green_Serenity_0924014250_texture.glb',
    targetHeight: 2.5,
  },
  snake_plant_white: {
    label: 'Snake Plant (White Pot)',
    url: '/models/Snake_Plant_in_White.glb',
    targetHeight: 2.5,
  },
  tropical_elegance: {
    label: 'Tropical Elegance',
    url: '/models/Tropical_Elegance_0924015639_texture.glb',
    targetHeight: 2.5,
  },
  verdant_elegance: {
    label: 'Verdant Elegance',
    url: '/models/Verdant_Elegance_0909080450_texture.glb',
    targetHeight: 2.5,
  },
  beige_serenity_sofa: {
    label: 'Beige Serenity Sofa',
    url: '/models/Beige_Serenity_Sofa_0925054256_texture.glb',
    targetHeight: 1.6, // 필요시 소파 크기 맞춰 조정
    wallSnap: true, // ← 벽 스냅 대상임을 표시
  },
  beige_serenity_sofa: {
    label: 'Beige Serenity Sofa',
    url: '/models/Beige_Serenity_Sofa_0925054256_texture.glb',
  ...
  modern_minimalist_coffee: {
    label: 'Modern Minimalist Coffee Table',
    url: '/models/Modern_Minimalist_Cof_0929014028_texture.glb',
    targetHeight: 1.2, // 테이블 높이(대략 42cm)
    showInToolbar: false, // 버튼 숨김
  },
  modern_sideboard: {
    label: 'Modern Sideboard',
    url: '/models/Modern_Sideboard_0929025836_texture.glb',
    targetHeight: 1.5, // 전체 높이(대략 80cm)
    showInToolbar: false, // 버튼 숨김
  },
  television_display: {
    label: 'Television Display',
    url: '/models/Television_Display_Si_0929030548_texture.glb',
    targetHeight: 3, // TV 전체 높이(대략 60cm)
    showInToolbar: false, // 버튼 숨김
  },
  modern_console_table: {
    label: 'Elegant Console Table',
    url: '/models/Elegant_Console_Table_0929035335_texture.glb',
    targetHeight: 2, // 테이블 약 75cm
    showInToolbar: false,
  },
  flower_vase: {
    label: 'Blossoming Beauty (Vase)',
    url: '/models/Blossoming_Beauty_0929035426_texture.glb',
...
;(async function spawnInitialSofaAndTable() {
  // 1) 소파
  const sofaCfg = MODEL_MAP.beige_serenity_sofa
  if (!sofaCfg) return
  ...
  // 2) 테이블 – 소파 앞에 자동 배치
  const tableCfg = MODEL_MAP.modern_minimalist_coffee
  if (!tableCfg) return
...
;(async function spawnSideboardAndTV() {
  ...
  // 1) 거실장
  const boardCfg = MODEL_MAP.modern_sideboard
  if (!boardCfg) return
  ...
  // 2) TV (거실장 위)
  const tvCfg = MODEL_MAP.television_display
  if (!tvCfg) return
...
;(async function spawnConsoleTableAndVaseOnFrontLeft() {
  ...
  // 1) 콘솔 테이블
  const tCfg = MODEL_MAP.modern_console_table
  if (!tCfg) return
